...
public abstract class MicroService implements Runnable {
    ...
    private static MessageBusImpl mBus = MessageBusImpl.getInstance();
    private static Diary diary = Diary.getInstance();
    private static boolean terminate = false;
    private ConcurrentHashMap<String, Callback<?>> callMap = new ConcurrentHashMap<String, Callback<?>>();
    /**
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        ...
        callMap.putIfAbsent(sType, callback);
    }
    ...
    protected final void terminate() {
        ...
        diary.setTerminate(this, System.currentTimeMillis());
    }
    ...
    public final void run() {
        mBus.register(this);
        mBus.subscribeBroadcast(TerminationBroadcast.class, this);
        initialize();
        while(!terminate){
            try {
                Message m = mBus.awaitMessage(this);
                if(m.getClass().getName().equals("bgu.spl.mics.application.messages.TerminationBroadcast")){ //May need to change to equals to
                   break;
                }
                else {
                    String mType = m.getClass().getName();
                    Callback callFunction = callMap.get(mType);
                    callFunction.call(m); //Here we call the callback function
                }
